...
function ScanReport() {
  ...
  const handleSaveReport = async () => {
    if (!isLoggedIn) {
      ...
      setIsLoggedIn(true); // Simulating login (Replace with actual login flow)
      return;
    }
    ...
    try {
      const response = await fetch(SAVE_REPORT_URL, {
        ...
        headers: {
          ...
          "Authorization": "Bearer your-token-here", // Replace with actual auth token
        },
        ...
        {analysisResult && (
          <div className="space-y-6">
            ...
            <div className="p-4 bg-gray-50 rounded-lg">
              <p className="flex items-center gap-2 text-gray-700 mb-4"><FileText size={18} /><span className="font-medium">Extracted Text:</span> {analysisResult.text}</p>
              <div className="grid gap-6 md:grid-cols-2">
                <div className="space-y-2">
                  <h3 className="flex items-center gap-2 font-medium text-gray-800"><List size={18} /> Key Findings</h3>
                  <ul className="list-disc list-inside text-gray-700 pl-2">{analysisResult.analysis.keyFindings.map((finding, index) => (<li key={`finding-${index}`} className="mb-1">{finding}</li>))}</ul>
                </div>
                <div className="space-y-2">
                  <h3 className="flex items-center gap-2 font-medium text-gray-800"><Clock size={18} /> Recommendations</h3>
                  <ul className="list-disc list-inside text-gray-700 pl-2">{analysisResult.analysis.recommendations.map((rec, index) => (<li key={`recommendation-${index}`} className="mb-1">{rec}</li>))}</ul>
                </div>
                <div className="space-y-2">
                  <h3 className="flex items-center gap-2 font-medium text-gray-800"><AlertCircle size={18} /> Urgent Concerns</h3>
                  <ul className="list-disc list-inside text-gray-700 pl-2">{analysisResult.analysis.urgentConcerns.map((concern, index) => (<li key={`concern-${index}`} className="mb-1">{concern}</li>))}</ul>
                </div>
                <div className="space-y-2">
                  <h3 className="font-medium text-gray-800">Simplified Explanation</h3>
                  <p className="text-gray-700">{analysisResult.analysis.simplifiedExplanation}</p>
                </div>
              </div>
            </div>
